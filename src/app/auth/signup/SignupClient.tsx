...
export default function SignupClient({ nextPath }: { nextPath: string }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(
        safeNextPath ? `/auth/signup-success?next=${encodeURIComponent(safeNextPath)}` : "/auth/signup-success",
      );
    } catch (err) {
  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-gradient-to-br from-slate-50 to-slate-100 px-4">
      <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-8 shadow-lg">
        ...
        <p className="mt-2 text-sm text-slate-600">Start your blog journey</p>
